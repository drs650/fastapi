...
# print(BASE_DIR) --> C:\Users\Administrator\bigdata2026\fastapi\Todo_project\ml
...
def build_pipeline() -> Pipeline:
    """
    TFidVevtorizer(텍스트 -> 숫자 벡터로 변환)
    로지스틱 회귀를 하나의 Pipeline 객체로 묶는다.
    --> 왜 묶어??
        서빙(FastAPI) 쪽에서 TfidVevtorizer, 예측도 해야 하고 ... 단계를 직접 호출해야 한다.
...
def get_next_version(artifacts_dir: Path) -> int:
    ...
    versions = [int(p.stem.stplit('_v')[-1]) for p in existing]
