...
# 2. 테스트 전용 DB를 설정
TEST_DATAVASE_URL = 'postgresql+psycopg2://postgres:1234@localhost:5432/reviewdb_test'

# 테스트용 엔진 --> reviewdb_test에 접속
test_engine = create_engine(TEST_DATAVASE_URL)
...
def test_read_users(client):
    """유저 전체 목록 조회 -> 200응답, 리스트 타입, 1명 이상 확인"""
...
def test_create_item_for_user(client):
    """1번 유저의 아이템 생성 -> 200응답, title 일치, owner_id 자동 설정 확인"""
    response = client.post(
