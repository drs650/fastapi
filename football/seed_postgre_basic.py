...
def parse_date(value: str) -> date:
    ...
    CSV에서 읽은 날짜 문자열을 Python date 객체로 변환한다.
        ...
        models.py의 last_changed_date 컬럼은 SQLAlchemy Date 타입니다.
...
def read_rows(filename: str) -> list[dict[str, str]]:
    ...
    csv.DictReader는 첫 줄의 컬럼명울 key로 사용하므로
...
def seed() -> None:
    ...
    with Session(engine) as session:
        ...
        # 2. league 테이블 적재 - league는 team에서 참조
        leagues = [
            models.League(
...
if __name__ == "__main__":
    ...
    # 다른 파일에서 import할 때 DB 초기화가 자동 실행되는 것을 막아준다.
    seed()
